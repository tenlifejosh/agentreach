...
async def upload_file(
    ...
    if trigger_selector:
        try:
            ...
            file_chooser = getattr(fc_info, "value", None)
            if inspect.isawaitable(file_chooser):
                file_chooser = await file_chooser
            if file_chooser is None:
                raise RuntimeError("File chooser did not provide a chooser object")
    ...
    try:
        drop_zone_selector = '[data-testid*="upload"], .upload-zone, .dropzone, [class*="drop"]'
        drop_zone = page.locator(drop_zone_selector).first
